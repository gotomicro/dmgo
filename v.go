...
func (innerRows *innerRows) HasNextResultSet() bool {
	...
	innerRows.nextExecInfo, err = innerRows.dmStmt.dmConn.Access.Dm_build_1401(innerRows.dmStmt, 0)
	if err != nil {
		panic(err)
	}
...
func (innerRows *innerRows) fetchData(startPos int64) bool {
	execInfo, err := innerRows.dmStmt.dmConn.Access.Dm_build_1408(innerRows, startPos)
	if err != nil {
		panic(err)
	}
	...
	for i, column := range innerRows.columns {
		...
		if i <= len(dest)-1 {
			if column.colType == CURSOR {
				...
				tmpExecInfo, err = innerRows.dmStmt.dmConn.Access.Dm_build_1401(innerRows.dmStmt, 1)
				if err != nil {
					return err
				}
