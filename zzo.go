...
func (sv TypeData) makeupObjToArr(obj interface{}, objDesc *TypeDescriptor) ([]interface{}, error) {
	...
	if subType == BINARY || subType == VARBINARY || subType == BIT {
		...
		rs := Dm_build_1257.Dm_build_1473(strRet, objDesc.getServerEncoding(), objDesc.m_conn)
		for i := 0; i < prec; i++ {
			ret[i] = rs[i]
		}
...
func (sv TypeData) typeDataToBytes(data *TypeData, desc *TypeDescriptor) ([]byte, error) {
	...
	if nil == data.m_dumyData {
		...
		Dm_build_1257.Dm_build_1258(innerData, 0, byte(0))
		Dm_build_1257.Dm_build_1258(innerData, 1, byte(0))
	...
	case ARRAY:
		...
		Dm_build_1257.Dm_build_1258(result, 0, byte(0))
		offset = 1

		Dm_build_1257.Dm_build_1258(result, offset, byte(1))
		offset += 1
	...
	case BOOLEAN:
		...
		if data.m_dataBuf != nil && len(data.m_dataBuf) > 0 {
			Dm_build_1257.Dm_build_1258(innerData, 1, data.m_dataBuf[0])
		} else {
			Dm_build_1257.Dm_build_1258(innerData, 1, byte(0))
		}
		return innerData, nil
	...
	default:
		...
		Dm_build_1257.Dm_build_1268(result, offset, int16(len(innerData)))
		offset += 2
...
func (sv TypeData) convertLobToBytes(value interface{}, dtype int, serverEncoding string) ([]byte, error) {
	...
	if dtype == BLOB {
		...
		if ok {
			...
			Dm_build_1257.Dm_build_1273(ret, 0, int32(l))
	...
	if dtype == CLOB {
		...
		if ok {
			...
			tmp = Dm_build_1257.Dm_build_1473(subString, serverEncoding, nil)
...
func (sv TypeData) sarrayToBytes(data *DmArray, desc *TypeDescriptor) ([]byte, error) {
	...
	Dm_build_1257.Dm_build_1273(rdata, off, int32(totalLen))
	off += ULINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, off, int32(data.m_arrDesc.getLength()))
	off += ULINT_SIZE
...
func (sv TypeData) ctlnToBytes(data *DmArray, desc *TypeDescriptor) ([]byte, error) {
	...
	Dm_build_1257.Dm_build_1258(rdata, offset, byte(0))
	offset += BYTE_SIZE

	offset += ULINT_SIZE

	Dm_build_1257.Dm_build_1268(rdata, offset, int16(desc.getCltnType()))
	offset += USINT_SIZE

	Dm_build_1257.Dm_build_1268(rdata, offset, int16(desc.m_arrObj.getDType()))
	offset += USINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, offset, int32(len(data.m_arrData)))
	offset += ULINT_SIZE
	...
	Dm_build_1257.Dm_build_1273(rdata, BYTE_SIZE, int32(offset))
...
func (sv TypeData) arrayToBytes(data *DmArray, desc *TypeDescriptor) ([]byte, error) {
	...
	Dm_build_1257.Dm_build_1273(rdata, 0, int32(totalLen))
	offset := ULINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, offset, int32(len(data.m_arrData)))
	offset += ULINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, offset, 0)
	offset += ULINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, offset, int32(data.m_objCount))
	offset += ULINT_SIZE

	Dm_build_1257.Dm_build_1273(rdata, offset, int32(data.m_strCount))
	offset += ULINT_SIZE

	for i := 0; i < total; i++ {
		Dm_build_1257.Dm_build_1273(rdata, offset, int32(data.m_objStrOffs[i]))
		offset += ULINT_SIZE
	}
...
func (sv TypeData) structToBytes(data *DmStruct, desc *TypeDescriptor) ([]byte, error) {
	...
	Dm_build_1257.Dm_build_1273(rdata, offset, int32(totalLen))
	offset += ULINT_SIZE
...
func (sv TypeData) bytesToBlob(val []byte, out *TypeData, desc *TypeDescriptor) (*TypeData, error) {
	offset := out.m_offset
	l := Dm_build_1257.Dm_build_1359(val, offset)
	offset += ULINT_SIZE

	tmp := Dm_build_1257.Dm_build_1408(val, offset, int(l))
	offset += int(l)
	out.m_offset = offset
...
func (sv TypeData) bytesToClob(val []byte, out *TypeData, desc *TypeDescriptor, serverEncoding string) (*TypeData, error) {
	...
	return newTypeData(newClobOfLocal(Dm_build_1257.Dm_build_1414(tmp, 0, len(tmp), serverEncoding, desc.m_conn), desc.m_conn), tmp), nil
}
...
func (sv TypeData) bytesToTypeData(val []byte, out *TypeData, desc *TypeDescriptor) (*TypeData, error) {
	...
	null_flag := Dm_build_1257.Dm_build_1350(val, offset)
	offset += 1
	...
	if desc.getDType() == BOOLEAN {
		...
		tmp := Dm_build_1257.Dm_build_1408(val, offset-1, 1)
	...
	case CLASS:
		if null_flag&byte(1) != byte(0) {
			...
			if out.m_offset > offset {
				retDataBuf = Dm_build_1257.Dm_build_1408(val, offset, out.m_offset-offset)
			}
...
func (sv TypeData) checkObjExist(val []byte, out *TypeData) bool {
	offset := out.m_offset
	exist_flag := Dm_build_1257.Dm_build_1350(val, offset)
	offset += 1
...
func (sv TypeData) findObjByPackId(val []byte, out *TypeData) (*DmStruct, error) {
	...
	pack_id := int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE
...
func (sv TypeData) bytesToObj_EXACT(val []byte, out *TypeData, desc *TypeDescriptor) (*DmStruct, error) {
	...
	strOut.m_dataBuf = Dm_build_1257.Dm_build_1408(val, offset, out.m_offset-offset)
...
func (sv TypeData) bytesToNestTab(val []byte, out *TypeData, desc *TypeDescriptor) (*DmArray, error) {
	...
	count := Dm_build_1257.Dm_build_1359(val, offset)
	offset += ULINT_SIZE
	...
	arrOut.m_dataBuf = Dm_build_1257.Dm_build_1408(val, offset, out.m_offset-offset)
...
func (sv TypeData) bytesToClnt(val []byte, out *TypeData, desc *TypeDescriptor) (*DmArray, error) {
	...
	cltn_type := Dm_build_1257.Dm_build_1354(val, offset)
	offset += USINT_SIZE
...
func (sv TypeData) bytesToArray(val []byte, out *TypeData, desc *TypeDescriptor) (*DmArray, error) {
	...
	arrOut.m_bufLen = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += 4

	arrOut.m_itemCount = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE

	arrOut.m_itemSize = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE

	arrOut.m_objCount = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE

	arrOut.m_strCount = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE

	total := arrOut.m_objCount + arrOut.m_strCount
	arrOut.m_objStrOffs = make([]int, total)
	for i := 0; i < total; i++ {
		arrOut.m_objStrOffs[i] = int(Dm_build_1257.Dm_build_1359(val, offset))
		offset += 4
	}
...
func (sv TypeData) bytesToRecord(val []byte, out *TypeData, desc *TypeDescriptor) (*DmStruct, error) {
	...
	strOut := newDmStructByTypeData(nil, desc)
	strOut.m_bufLen = int(Dm_build_1257.Dm_build_1359(val, offset))
	offset += ULINT_SIZE
...
func (sv TypeData) objBlob_GetChkBuf(buf []byte, typeData *TypeData) {
	...
	l := int(Dm_build_1257.Dm_build_1359(buf, offset))
	offset += ULINT_SIZE

	typeData.m_objBlobDescBuf = Dm_build_1257.Dm_build_1408(buf, offset, l)
	offset += l
...
func (sv TypeData) objBlobToBytes(lobBuf []byte, desc *TypeDescriptor) ([]byte, error) {
	...
	magic := Dm_build_1257.Dm_build_1359(lobBuf, offset)
	offset += ULINT_SIZE
	...
	descLen := int(Dm_build_1257.Dm_build_1359(lobBuf, offset))
	offset += ULINT_SIZE
	descBuf := Dm_build_1257.Dm_build_1408(lobBuf, offset, descLen)
...
func (sv TypeData) convertBytes2BaseData(val []byte, out *TypeData, desc *TypeDescriptor) (*TypeData, error) {
	offset := out.m_offset
	isNull := false
	valueLen := int(Dm_build_1257.Dm_build_1381(val, offset))
	offset += USINT_SIZE

	if valueLen == int(Dm_build_667) {
		valueLen = 0
		isNull = true
	}

	if -1 == valueLen {
		valueLen = int(Dm_build_1257.Dm_build_1359(val, offset))
		offset += ULINT_SIZE
	}
	...
	var err error
	temp := Dm_build_1257.Dm_build_1408(val, offset, valueLen)
	offset += valueLen
	out.m_offset = offset
...
func (td *TypeData) toBytesFromDmArray(x *DmArray, typeDesc *TypeDescriptor) ([]byte, error) {
	...
	Dm_build_1257.Dm_build_1273(ret, 0, OBJ_BLOB_MAGIC)
	Dm_build_1257.Dm_build_1273(ret, ULINT_SIZE, int32(len(desc)))
