...
func dmdtDecodeFast(value []byte) []int {
	...
	if len(value) == 3 {
		...
		dt[OFFSET_YEAR] = int(Dm_build_623.Dm_build_720(value, 0)) & 0x7FFF
		...
		if len(value) > 5 {
			dt[OFFSET_TIMEZONE] = int(Dm_build_623.Dm_build_720(value, 5))
		}
	} else {
		...
		if len(value) > 8 {
			dt[OFFSET_TIMEZONE] = int(Dm_build_623.Dm_build_720(value, 8))
		}
	}
	return dt
}
...
func dmdtDecodeBdta(value []byte) []int {
	dt := make([]int, DT_LEN)
	dt[OFFSET_YEAR] = int(Dm_build_623.Dm_build_720(value, 0))
	...
	dt[OFFSET_TIMEZONE] = int(Dm_build_623.Dm_build_720(value, 10))
	return dt
}
...
func encode(dt []int, dtype int, scale int, lTz int) ([]byte, error) {
	...
	if dtype == DATE {
		...
		Dm_build_623.Dm_build_634(ret, 8, int16(tz))
	} else if dtype == TIME {
		ret = make([]byte, 5)
		...
		Dm_build_623.Dm_build_634(ret, 5, int16(tz))
	}
