...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_1434(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_1359(st, Dm_build_85)
	if err != nil {
		return err
	}
...
func NewDmStmt(conn *DmConnection, sql string) (*DmStatement, error) {
	...
	if s == nil {
		...
		if conn.stmtPool != nil && len(conn.stmtPool) > 0 {
			...
			err := conn.Access.Dm_build_1341(s)
			if err != nil {
				return nil, err
			}
		}
...
func (stmt *DmStatement) free() error {
	...
	err := stmt.dmConn.Access.Dm_build_1346(int32(stmt.id))
	if err != nil {
		return err
	}
	delete(stmt.dmConn.stmtMap, stmt.id)
	...
	nextSwitch:
		if arg == nil {
			if stmt.params[i].colType == CURSOR && stmt.params[i].cursorStmt == nil {
				...
				err = stmt.params[i].cursorStmt.dmConn.Access.Dm_build_1341(stmt.params[i].cursorStmt)
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_1391(stmt, bytes, false)
	if err != nil {
		return err
	}
			...
			if stmt.curRowBindIndicator[i]&BIND_OUT == BIND_OUT {
				if outParamData == nil {
					if arg, ok := args[i].(*driver.Rows); ok && stmt.params[i].colType == CURSOR {
						...
						if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_1401(stmt.params[i].cursorStmt, 1); err != nil {
							return err
						}
				...
				nextSwitch:
					...
					case *driver.Rows:
						if stmt.params[i].colType == CURSOR {
							var tmpExecInfo *execInfo
							tmpExecInfo, err = stmt.dmConn.Access.Dm_build_1401(stmt.params[i].cursorStmt, 1)
							if err != nil {
								return err
							}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_1380(stmt, bytes)
	return err
}
