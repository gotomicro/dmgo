...
func newDmIntervalYMByBytes(bytes []byte) *DmIntervalYM {
	...
	ym.scaleForSvr = int(Dm_build_1257.Dm_build_1359(bytes, 8))
	ym.leadScale = (ym.scaleForSvr >> 4) & 0x0000000F
	ym._type = bytes[9]
	switch ym._type {
	case QUA_Y:
		ym.years = int(Dm_build_1257.Dm_build_1359(bytes, 0))
	case QUA_YM:
		ym.years = int(Dm_build_1257.Dm_build_1359(bytes, 0))
		ym.months = int(Dm_build_1257.Dm_build_1359(bytes, 4))
	case QUA_MO:
		ym.months = int(Dm_build_1257.Dm_build_1359(bytes, 4))
	}
	return ym
...
func (ym *DmIntervalYM) encode(scale int) ([]byte, error) {
	...
	bytes := make([]byte, 12)
	Dm_build_1257.Dm_build_1273(bytes, 0, int32(year))
	Dm_build_1257.Dm_build_1273(bytes, 4, int32(month))
	Dm_build_1257.Dm_build_1273(bytes, 8, int32(scale))
	return bytes, nil
}
