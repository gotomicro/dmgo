...
func newBlobFromDB(value []byte, conn *DmConnection, column *column, fetchAll bool) *DmBlob {
	...
	blob.inRow = Dm_build_599.Dm_build_692(value, NBLOB_HEAD_IN_ROW_FLAG) == LOB_IN_ROW
	blob.blobId = Dm_build_599.Dm_build_706(value, NBLOB_HEAD_BLOBID)
	if !blob.inRow {
		blob.groupId = Dm_build_599.Dm_build_696(value, NBLOB_HEAD_OUTROW_GROUPID)
		blob.fileId = Dm_build_599.Dm_build_696(value, NBLOB_HEAD_OUTROW_FILEID)
		blob.pageNo = Dm_build_599.Dm_build_701(value, NBLOB_HEAD_OUTROW_PAGENO)
	}
	if conn.NewLobFlag {
		blob.tabId = Dm_build_599.Dm_build_701(value, NBLOB_EX_HEAD_TABLE_ID)
		blob.colId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_COL_ID)
		blob.rowId = Dm_build_599.Dm_build_706(value, NBLOB_EX_HEAD_ROW_ID)
		blob.exGroupId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_FPA_GRPID)
		blob.exFileId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_FPA_FILEID)
		blob.exPageNo = Dm_build_599.Dm_build_701(value, NBLOB_EX_HEAD_FPA_PAGENO)
	}
	...
	if blob.inRow {
		...
		if conn.NewLobFlag {
			Dm_build_599.Dm_build_655(blob.data, 0, value, NBLOB_EX_HEAD_SIZE, len(blob.data))
		} else {
			Dm_build_599.Dm_build_655(blob.data, 0, value, NBLOB_INROW_HEAD_SIZE, len(blob.data))
		}
	} else if fetchAll {
		blob.loadAllData()
	}
...
func (blob *DmBlob) Write(pos int, src []byte) (n int, err error) {
	...
	if blob.local || blob.fetchAll {
		...
		var writeLen, err = blob.connection.Access.dm_build_1494(blob, pos, src)
		if err != nil {
			return -1, err
		}
...
func (blob *DmBlob) Truncate(length int64) error {
	...
	if blob.local || blob.fetchAll {
		...
		Dm_build_599.Dm_build_655(tmp, 0, blob.data, 0, len(tmp))
		blob.data = tmp
		...
		blob.length, err = blob.connection.Access.dm_build_1508(&blob.lob, int(length))
		if err != nil {
			return err
		}
		if blob.groupId == -1 {
			tmp := make([]byte, blob.length)
			Dm_build_599.Dm_build_655(tmp, 0, blob.data, 0, int(blob.length))
			blob.data = tmp
		}
	}
	blob.modify = true
...
func (blob *DmBlob) getBytes(pos int64, length int32) ([]byte, error) {
	...
	if blob.local || blob.inRow || blob.fetchAll {
		...
		return blob.connection.Access.dm_build_1457(blob, int32(pos), length)
	}
}
...
func (blob *DmBlob) setLocalData(pos int, p []byte) {
	if pos+len(p) >= int(blob.length) {
		...
		Dm_build_599.Dm_build_655(tmp, 0, blob.data, 0, pos)
		Dm_build_599.Dm_build_655(tmp, pos, p, 0, len(p))
		blob.data = tmp
	} else {
		Dm_build_599.Dm_build_655(blob.data, pos, p, 0, len(p))
	}
	blob.length = int64(len(blob.data))
}
