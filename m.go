...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_739 {
	case Dm_build_736, Dm_build_737, Dm_build_738:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_703 && dc.dmConnector.enRsCache {
	...
	if execType == Dm_build_703 && info.hasResultSet {
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_704)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_703)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) driverQuery(query string) (*DmStatement, *DmRows, error) {
	...
	stmt.innerUsed = true
	stmt.innerExec = true
	info, err := dc.Access.Dm_build_425(stmt, Dm_build_703)
	if err != nil {
		return nil, nil, err
	}
	dc.lastExecInfo = info
