...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_1078 {
	case Dm_build_1075, Dm_build_1076, Dm_build_1077:
		conn.trxFinish = true
	default:
...
func (dmConn *DmConnection) init() {
	...
	dmConn.TrxStatus = Dm_build_1026
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_1042 && dc.dmConnector.enRsCache {
...
func (dc *DmConnection) close() error {
	...
	if dc.stmtPool != nil {
		for _, spi := range dc.stmtPool {
			err = dc.Access.Dm_build_752(spi.id)
			if err != nil {
				return err
			}
		}
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_1043)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_1042)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) driverQuery(query string) (*DmStatement, *DmRows, error) {
	...
	stmt.innerUsed = true
	stmt.innerExec = true
	info, err := dc.Access.Dm_build_775(stmt, Dm_build_1042)
	if err != nil {
		return nil, nil, err
	}
	dc.lastExecInfo = info
