...
func (typeDescriptor *TypeDescriptor) parseDescByName() error {
	...
	err = typeDescriptor.unpack(Dm_build_363(buf))
	if err != nil {
		return err
	}
	return nil
...
func pack(typeDesc *TypeDescriptor, msg *Dm_build_358) error {
	switch typeDesc.column.colType {
	case ARRAY, SARRAY:
	...
	msg.Dm_build_409(typeDesc.column.colType)

	msg.Dm_build_409(typeDesc.column.prec)

	msg.Dm_build_409(typeDesc.column.scale)
	return nil
}
...
func getPackArraySize(arrDesc *TypeDescriptor) (int, error) {
	...
	ret := Dm_build_1.Dm_build_214(name, serverEncoding, arrDesc.m_conn)
	l += len(ret)
...
func packArray(arrDesc *TypeDescriptor, msg *Dm_build_358) error {

	msg.Dm_build_409(arrDesc.column.colType)

	msg.Dm_build_465(arrDesc.m_sqlName.m_name, arrDesc.getServerEncoding(), arrDesc.m_conn)

	msg.Dm_build_409(int32(arrDesc.m_objId))

	msg.Dm_build_409(int32(arrDesc.m_objVersion))

	msg.Dm_build_409(int32(arrDesc.m_length))

	return pack(arrDesc.m_arrObj, msg)
}

func packRecord(strctDesc *TypeDescriptor, msg *Dm_build_358) error {

	msg.Dm_build_409(strctDesc.column.colType)

	msg.Dm_build_465(strctDesc.m_sqlName.m_name, strctDesc.getServerEncoding(), strctDesc.m_conn)

	msg.Dm_build_409(int32(strctDesc.m_objId))

	msg.Dm_build_409(int32(strctDesc.m_objVersion))

	msg.Dm_build_405(int16(strctDesc.m_size))
...
func getPackRecordSize(strctDesc *TypeDescriptor) (int, error) {
	...
	ret := Dm_build_1.Dm_build_214(name, serverEncoding, strctDesc.m_conn)
	l += len(ret)
...
func packClass(strctDesc *TypeDescriptor, msg *Dm_build_358) error {
	...
	if strctDesc.m_objId == 4 {
		...
		msg.Dm_build_409(int32(strctDesc.m_outerId))

		msg.Dm_build_409(int32(strctDesc.m_outerVer))

		msg.Dm_build_409(int32(strctDesc.m_subId))
...
func (typeDescriptor *TypeDescriptor) unpack(buffer *Dm_build_358) error {

	typeDescriptor.column.colType = buffer.Dm_build_483()
	...
	typeDescriptor.column.prec = buffer.Dm_build_483()

	typeDescriptor.column.scale = buffer.Dm_build_483()
	return nil
}

func (typeDescriptor *TypeDescriptor) unpackArray(buffer *Dm_build_358) error {

	typeDescriptor.m_sqlName.m_name = buffer.Dm_build_533(typeDescriptor.getServerEncoding(), typeDescriptor.m_conn)

	typeDescriptor.m_sqlName.m_schId = int(buffer.Dm_build_483())

	typeDescriptor.m_sqlName.m_packId = int(buffer.Dm_build_483())

	typeDescriptor.m_objId = int(buffer.Dm_build_483())

	typeDescriptor.m_objVersion = int(buffer.Dm_build_483())

	typeDescriptor.m_length = int(buffer.Dm_build_483())
...
func (typeDescriptor *TypeDescriptor) unpackRecord(buffer *Dm_build_358) error {
	...
	typeDescriptor.m_size = int(buffer.Dm_build_498())
...
func (typeDescriptor *TypeDescriptor) unpackClnt_nestTab(buffer *Dm_build_358) error {

	typeDescriptor.m_maxCnt = int(buffer.Dm_build_483())
...
func (typeDescriptor *TypeDescriptor) unpackClnt(buffer *Dm_build_358) error {

	typeDescriptor.m_outerId = int(buffer.Dm_build_483())

	typeDescriptor.m_outerVer = int(buffer.Dm_build_483())

	typeDescriptor.m_subId = int(buffer.Dm_build_498())

	typeDescriptor.m_cltnType = int(buffer.Dm_build_498())
...
func (typeDescriptor *TypeDescriptor) unpackClass(buffer *Dm_build_358) error {
