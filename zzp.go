...
func (typeDescriptor *TypeDescriptor) parseDescByName() error {
	...
	err = typeDescriptor.unpack(Dm_build_985(buf))
	if err != nil {
		return err
	}
	return nil
...
func pack(typeDesc *TypeDescriptor, msg *Dm_build_980) error {
	switch typeDesc.column.colType {
	case ARRAY, SARRAY:
	...
	msg.Dm_build_1031(typeDesc.column.colType)

	msg.Dm_build_1031(typeDesc.column.prec)

	msg.Dm_build_1031(typeDesc.column.scale)
	return nil
}
...
func getPackArraySize(arrDesc *TypeDescriptor) (int, error) {
	...
	ret := Dm_build_623.Dm_build_836(name, serverEncoding, arrDesc.m_conn)
	l += len(ret)
...
func packArray(arrDesc *TypeDescriptor, msg *Dm_build_980) error {

	msg.Dm_build_1031(arrDesc.column.colType)

	msg.Dm_build_1087(arrDesc.m_sqlName.m_name, arrDesc.getServerEncoding(), arrDesc.m_conn)

	msg.Dm_build_1031(int32(arrDesc.m_objId))

	msg.Dm_build_1031(int32(arrDesc.m_objVersion))

	msg.Dm_build_1031(int32(arrDesc.m_length))

	return pack(arrDesc.m_arrObj, msg)
}

func packRecord(strctDesc *TypeDescriptor, msg *Dm_build_980) error {

	msg.Dm_build_1031(strctDesc.column.colType)

	msg.Dm_build_1087(strctDesc.m_sqlName.m_name, strctDesc.getServerEncoding(), strctDesc.m_conn)

	msg.Dm_build_1031(int32(strctDesc.m_objId))

	msg.Dm_build_1031(int32(strctDesc.m_objVersion))

	msg.Dm_build_1027(int16(strctDesc.m_size))
...
func getPackRecordSize(strctDesc *TypeDescriptor) (int, error) {
	...
	ret := Dm_build_623.Dm_build_836(name, serverEncoding, strctDesc.m_conn)
	l += len(ret)
...
func packClass(strctDesc *TypeDescriptor, msg *Dm_build_980) error {
	...
	if strctDesc.m_objId == 4 {
		...
		msg.Dm_build_1031(int32(strctDesc.m_outerId))

		msg.Dm_build_1031(int32(strctDesc.m_outerVer))

		msg.Dm_build_1031(int32(strctDesc.m_subId))
...
func (typeDescriptor *TypeDescriptor) unpack(buffer *Dm_build_980) error {

	typeDescriptor.column.colType = buffer.Dm_build_1105()
	...
	typeDescriptor.column.prec = buffer.Dm_build_1105()

	typeDescriptor.column.scale = buffer.Dm_build_1105()
	return nil
}

func (typeDescriptor *TypeDescriptor) unpackArray(buffer *Dm_build_980) error {

	typeDescriptor.m_sqlName.m_name = buffer.Dm_build_1155(typeDescriptor.getServerEncoding(), typeDescriptor.m_conn)

	typeDescriptor.m_sqlName.m_schId = int(buffer.Dm_build_1105())

	typeDescriptor.m_sqlName.m_packId = int(buffer.Dm_build_1105())

	typeDescriptor.m_objId = int(buffer.Dm_build_1105())

	typeDescriptor.m_objVersion = int(buffer.Dm_build_1105())

	typeDescriptor.m_length = int(buffer.Dm_build_1105())
...
func (typeDescriptor *TypeDescriptor) unpackRecord(buffer *Dm_build_980) error {
	...
	typeDescriptor.m_size = int(buffer.Dm_build_1120())
...
func (typeDescriptor *TypeDescriptor) unpackClnt_nestTab(buffer *Dm_build_980) error {

	typeDescriptor.m_maxCnt = int(buffer.Dm_build_1105())
...
func (typeDescriptor *TypeDescriptor) unpackClnt(buffer *Dm_build_980) error {

	typeDescriptor.m_outerId = int(buffer.Dm_build_1105())

	typeDescriptor.m_outerVer = int(buffer.Dm_build_1105())

	typeDescriptor.m_subId = int(buffer.Dm_build_1120())

	typeDescriptor.m_cltnType = int(buffer.Dm_build_1120())
...
func (typeDescriptor *TypeDescriptor) unpackClass(buffer *Dm_build_980) error {
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_array(offset int, desc *TypeDescriptor) (int, error) {
	...
	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, ARRAY)
	offset += USINT_SIZE

	Dm_build_623.Dm_build_639(typeDescriptor.m_descBuf, offset, int32(desc.m_length))
	offset += ULINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_record(offset int, desc *TypeDescriptor) (int, error) {
	...
	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, PLTYPE_RECORD)
	offset += USINT_SIZE

	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, int16(desc.m_size))
	offset += USINT_SIZE
	var err error
	for i := 0; i < desc.m_size; i++ {
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_buildin(offset int, desc *TypeDescriptor) int {
	...
	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, dtype)
	offset += USINT_SIZE

	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, int16(prec))
	offset += USINT_SIZE

	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, int16(scale))
	offset += USINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_class_clnt(offset int, desc *TypeDescriptor) (int, error) {
	...
	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, int16(desc.m_cltnType))
	offset += USINT_SIZE

	Dm_build_623.Dm_build_639(typeDescriptor.m_descBuf, offset, int32(desc.getMaxCnt()))
	offset += ULINT_SIZE
...
func (typeDescriptor *TypeDescriptor) makeChkDesc_class(offset int, desc *TypeDescriptor) (int, error) {
	...
	Dm_build_623.Dm_build_634(typeDescriptor.m_descBuf, offset, CLASS)
	offset += USINT_SIZE
	...
	if isClnt {
		Dm_build_623.Dm_build_624(typeDescriptor.m_descBuf, offset, byte(1))
	} else {
		Dm_build_623.Dm_build_624(typeDescriptor.m_descBuf, offset, byte(0))
	}
