...
func (clob *DmClob) WriteString(pos int, s string) (n int, err error) {
	...
	if clob.local || clob.fetchAll {
		...
		var writeLen, err = clob.connection.Access.dm_build_884(clob, pos, s, clob.serverEncoding)
		if err != nil {
			return -1, err
		}
...
func (clob *DmClob) Truncate(length int64) error {
	...
	if clob.local || clob.fetchAll {
		...
		clob.length, err = clob.connection.Access.dm_build_914(&clob.lob, int(length))
		if err != nil {
			return err
		}
		if clob.groupId == -1 {
...
func (clob *DmClob) getSubString(pos int64, len int32) (string, error) {
	...
	if clob.local || clob.inRow || clob.fetchAll {
		...
		return clob.connection.Access.dm_build_873(clob, int32(pos), len)
	}
}
