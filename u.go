...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_490(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_415(st, Dm_build_702)
	if err != nil {
		return err
	}
	st.prepared = true
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_704)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_703)
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_447(stmt, bytes, false)
	if err != nil {
		return err
	}
		...
		for i, outParamData := range stmt.execInfo.outParamDatas {
			...
			if sc, ok := args[i].(sql.Scanner); ok {
				...
				case CURSOR:
					...
					if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_457(stmt.bindParams[i].cursorStmt, 1); err != nil {
						return err
					}
				...
				case *driver.Rows:
					if stmt.bindParams[i].colType == CURSOR {
						var tmpExecInfo *execRetInfo
						tmpExecInfo, err = stmt.dmConn.Access.Dm_build_457(stmt.bindParams[i].cursorStmt, 1)
						if err != nil {
							return err
						}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	if stmt.execInfo.retSqlType == Dm_build_717 || stmt.execInfo.retSqlType == Dm_build_722 {
	...
	if stmt.paramCount > 0 && args != nil && len(args) > 0 {
		...
		if len(args) == 1 || stmt.dmConn.dmConnector.batchType == 2 ||
			(stmt.dmConn.dmConnector.batchNotOnCall && stmt.execInfo.retSqlType == Dm_build_718) {
...
func (stmt *DmStatement) executeBatchByRow(args []driver.Value) (err error) {
	...
	for i := 0; i < count; i++ {
		tmpExecInfo, err := stmt.dmConn.Access.Dm_build_447(stmt, args[i].([]interface{}), stmt.preExec || i != 0)
		if err == nil {
