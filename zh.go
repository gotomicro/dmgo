...
func (G2DB g2db) fromBool(val bool, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BINARY, VARBINARY, BLOB:
		if val {
			return Dm_build_599.Dm_build_774(byte(1)), nil
		} else {
			return Dm_build_599.Dm_build_774(byte(0)), nil
		}
	}
...
func (G2DB g2db) fromInt64(val int64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case TINYINT:
		...
		return Dm_build_599.Dm_build_774(byte(val)), nil
	case SMALLINT:
		...
		return Dm_build_599.Dm_build_777(int16(val)), nil
	case INT:
		...
		return Dm_build_599.Dm_build_780(int32(val)), nil
	case BIGINT:
		...
		return Dm_build_599.Dm_build_783(int64(val)), nil
	case REAL:
		...
		return Dm_build_599.Dm_build_786(float32(val)), nil
	case DOUBLE:
		return Dm_build_599.Dm_build_789(float64(val)), nil
	case DECIMAL:
	...
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		return Dm_build_599.Dm_build_809(strconv.FormatInt(val, 10), conn.getServerEncoding(), conn), nil
	case BINARY, VARBINARY, BLOB:
...
func (G2DB g2db) fromFloat32(val float32, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BOOLEAN, BIT:
		if val == 0.0 {
			return Dm_build_599.Dm_build_774(0), nil
		}
		return Dm_build_599.Dm_build_774(1), nil
	case TINYINT:
	...
	case REAL:
		...
		return Dm_build_599.Dm_build_786(val), nil
	case DOUBLE:
		return Dm_build_599.Dm_build_789(float64(val)), nil
	case DECIMAL:
		...
		return d.encodeDecimal()
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		return Dm_build_599.Dm_build_809(strconv.FormatFloat(float64(val), 'f', -1, 32), conn.getServerEncoding(), conn), nil
	}
	return nil, ECGO_DATA_CONVERTION_ERROR.throw()
}
...
func (G2DB g2db) fromFloat64(val float64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case DECIMAL:
		...
		return d.encodeDecimal()
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		return Dm_build_599.Dm_build_809(strconv.FormatFloat(val, 'f', -1, 64), conn.getServerEncoding(), conn), nil
	}
	return nil, ECGO_DATA_CONVERTION_ERROR.throw()
}
...
func (G2DB g2db) fromString(val string, param parameter, conn *DmConnection) (interface{}, error) {
	...
	case CHAR, VARCHAR2, VARCHAR:
		return Dm_build_599.Dm_build_809(val, conn.getServerEncoding(), conn), nil
	case CLOB:
...
func (G2DB g2db) fromBigInt(val *big.Int, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BOOLEAN, BIT:
		if val.Sign() == 0 {
			ret = Dm_build_599.Dm_build_774(0)
		} else {
			ret = Dm_build_599.Dm_build_774(1)
		}
	case TINYINT:
		...
		ret = Dm_build_599.Dm_build_774(byte(val.Int64()))
	case SMALLINT:
		...
		ret = Dm_build_599.Dm_build_777(int16(val.Int64()))
	case INT:
		...
		ret = Dm_build_599.Dm_build_780(int32(val.Int64()))
	case BIGINT:
		...
		ret = Dm_build_599.Dm_build_783(val.Int64())
	case REAL:
		...
		ret = Dm_build_599.Dm_build_786(float32(val.Int64()))
	case DOUBLE:
		ret = Dm_build_599.Dm_build_789(float64(val.Int64()))
	...
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		ret = Dm_build_599.Dm_build_809(val.String(), conn.getServerEncoding(), conn)
	default:
		return nil, ECGO_DATA_CONVERTION_ERROR.throw()
	}
	return ret, nil
...
func (G2DB g2db) fromBigFloat(val *big.Float, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case TINYINT:
		...
		ret = Dm_build_599.Dm_build_774(byte(f))
	case SMALLINT:
		f, _ := val.Float64()
		...
		ret = Dm_build_599.Dm_build_777(int16(f))
	case INT:
		f, _ := val.Float64()
		...
		ret = Dm_build_599.Dm_build_780(int32(f))
	case BIGINT:
		f, _ := val.Float64()
		...
		ret = Dm_build_599.Dm_build_783(int64(f))
	case REAL:
		f, _ := val.Float64()
		...
		ret = Dm_build_599.Dm_build_786(float32(f))
	case DOUBLE:
		f, _ := val.Float64()
		ret = Dm_build_599.Dm_build_789(f)
	case DECIMAL:
		d, err := newDecimal(val, int(param.prec), int(param.scale))
		if err != nil {
			return nil, err
	...
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		ret = Dm_build_599.Dm_build_809(val.Text('f', int(param.scale)), conn.getServerEncoding(), conn)
	default:
		return nil, ECGO_DATA_CONVERTION_ERROR.throw()
	}
	return ret, nil
...
func (G2DB g2db) fromDecimal(val DmDecimal, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case TINYINT:
		...
		ret = Dm_build_599.Dm_build_774(byte(val.ToBigInt().Int64()))
	case SMALLINT:
		if err := G2DB.checkSmallint(val); err != nil {
			return nil, err
		}
		ret = Dm_build_599.Dm_build_777(int16(val.ToBigInt().Int64()))
	case INT:
		if err := G2DB.checkInt(val); err != nil {
			return nil, err
		}
		ret = Dm_build_599.Dm_build_780(int32(val.ToBigInt().Int64()))
	case BIGINT:
		if err := G2DB.checkBigint(val); err != nil {
			return nil, err
		}
		ret = Dm_build_599.Dm_build_783(int64(val.ToBigInt().Int64()))
	case REAL:
		if err := G2DB.checkReal(val); err != nil {
			return nil, err
		}
		f, _ := val.ToBigFloat().Float32()
		ret = Dm_build_599.Dm_build_786(f)
	case DOUBLE:
	...
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		ret = Dm_build_599.Dm_build_809(val.ToBigFloat().Text('f', -1), conn.getServerEncoding(), conn)
	default:
		return nil, ECGO_DATA_CONVERTION_ERROR.throw()
	}
	return ret, nil
...
func (G2DB g2db) fromTime(val time.Time, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		return Dm_build_599.Dm_build_809(val.Format("2006-01-02 15:04:05.999999999 -07:00"), conn.getServerEncoding(), conn), nil
	}
...
func (G2DB g2db) fromDmIntervalDT(val DmIntervalDT, param parameter, conn *DmConnection) ([]byte, error) {
	switch param.colType {
	case CHAR, VARCHAR2, VARCHAR, CLOB:
		return Dm_build_599.Dm_build_809(val.String(), conn.getServerEncoding(), conn), nil
	case INTERVAL_DT:
...
func (G2DB g2db) fromReader(val io.Reader, param parameter, conn *DmConnection) (interface{}, error) {
	...
	case CHAR, VARCHAR2, VARCHAR:
		...
		return Dm_build_599.Dm_build_809(string(bytesBuf.Bytes()), conn.getServerEncoding(), conn), nil
	case BINARY, VARBINARY:
		var bytesBuf = new(bytes.Buffer)
		if _, err := bytesBuf.ReadFrom(val); err != nil {
			return nil, err
...
func (G2DB g2db) string2Clob(val string, param parameter, conn *DmConnection) (interface{}, error) {
	return G2DB.changeOffRowData(param, Dm_build_599.Dm_build_809(val, conn.getServerEncoding(), conn), conn.getServerEncoding())
}
...
func (G2DB g2db) toInt32(val int32) []byte {
	bytes := make([]byte, 4)
	Dm_build_599.Dm_build_615(bytes, 0, val)
	return bytes
}

func (G2DB g2db) toInt64(val int64) []byte {
	bytes := make([]byte, 8)
	Dm_build_599.Dm_build_620(bytes, 0, val)
	return bytes
}

func (G2DB g2db) toFloat32(val float32) []byte {
	bytes := make([]byte, 4)
	Dm_build_599.Dm_build_625(bytes, 0, val)
	return bytes
}

func (G2DB g2db) toFloat64(val float64) []byte {
	bytes := make([]byte, 8)
	Dm_build_599.Dm_build_630(bytes, 0, val)
	return bytes
}
