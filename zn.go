...
func (lob *lob) getLengthFromHead(head []byte) int64 {
	return int64(Dm_build_885.Dm_build_987(head, NBLOB_HEAD_BLOB_LEN))
}
...
func (lob *lob) buildCtlData() (bytes []byte) {
	...
	Dm_build_885.Dm_build_886(bytes, NBLOB_HEAD_IN_ROW_FLAG, LOB_OFF_ROW)
	Dm_build_885.Dm_build_906(bytes, NBLOB_HEAD_BLOBID, lob.blobId)
	Dm_build_885.Dm_build_901(bytes, NBLOB_HEAD_BLOB_LEN, -1)

	Dm_build_885.Dm_build_896(bytes, NBLOB_HEAD_OUTROW_GROUPID, lob.groupId)
	Dm_build_885.Dm_build_896(bytes, NBLOB_HEAD_OUTROW_FILEID, lob.fileId)
	Dm_build_885.Dm_build_901(bytes, NBLOB_HEAD_OUTROW_PAGENO, lob.pageNo)

	if lob.connection.NewLobFlag {
		Dm_build_885.Dm_build_901(bytes, NBLOB_EX_HEAD_TABLE_ID, lob.tabId)
		Dm_build_885.Dm_build_896(bytes, NBLOB_EX_HEAD_COL_ID, lob.colId)
		Dm_build_885.Dm_build_906(bytes, NBLOB_EX_HEAD_ROW_ID, lob.rowId)
		Dm_build_885.Dm_build_896(bytes, NBLOB_EX_HEAD_FPA_GRPID, lob.exGroupId)
		Dm_build_885.Dm_build_896(bytes, NBLOB_EX_HEAD_FPA_FILEID, lob.exFileId)
		Dm_build_885.Dm_build_901(bytes, NBLOB_EX_HEAD_FPA_PAGENO, lob.exPageNo)
	}
	return
}
