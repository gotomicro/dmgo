...
func Compress(srcBuffer *Dm_build_953, offset int, length int, compressID int) ([]byte, error) {
	if compressID == Dm_build_83 {
		return snappy.Encode(nil, srcBuffer.Dm_build_1243(offset, length)), nil
	}
...
func GzlibCompress(srcBuffer *Dm_build_953, offset int, length int) ([]byte, error) {
	var ret bytes.Buffer
	var w = zlib.NewWriter(&ret)
	w.Write(srcBuffer.Dm_build_1243(offset, length))
	w.Close()
