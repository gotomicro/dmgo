...
func newClobFromDB(value []byte, conn *DmConnection, column *column, fetchAll bool) *DmClob {
	...
	clob.inRow = Dm_build_599.Dm_build_692(value, NBLOB_HEAD_IN_ROW_FLAG) == LOB_IN_ROW
	clob.blobId = Dm_build_599.Dm_build_706(value, NBLOB_HEAD_BLOBID)
	if !clob.inRow {
		clob.groupId = Dm_build_599.Dm_build_696(value, NBLOB_HEAD_OUTROW_GROUPID)
		clob.fileId = Dm_build_599.Dm_build_696(value, NBLOB_HEAD_OUTROW_FILEID)
		clob.pageNo = Dm_build_599.Dm_build_701(value, NBLOB_HEAD_OUTROW_PAGENO)
	}
	if conn.NewLobFlag {
		clob.tabId = Dm_build_599.Dm_build_701(value, NBLOB_EX_HEAD_TABLE_ID)
		clob.colId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_COL_ID)
		clob.rowId = Dm_build_599.Dm_build_706(value, NBLOB_EX_HEAD_ROW_ID)
		clob.exGroupId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_FPA_GRPID)
		clob.exFileId = Dm_build_599.Dm_build_696(value, NBLOB_EX_HEAD_FPA_FILEID)
		clob.exPageNo = Dm_build_599.Dm_build_701(value, NBLOB_EX_HEAD_FPA_PAGENO)
	}
	...
	if clob.inRow {
		if conn.NewLobFlag {
			clob.data = []rune(Dm_build_599.Dm_build_753(value, NBLOB_EX_HEAD_SIZE, int(clob.getLengthFromHead(value)), clob.serverEncoding, conn))
		} else {
			clob.data = []rune(Dm_build_599.Dm_build_753(value, NBLOB_INROW_HEAD_SIZE, int(clob.getLengthFromHead(value)), clob.serverEncoding, conn))
		}
...
func (clob *DmClob) WriteString(pos int, s string) (n int, err error) {
	...
	if clob.local || clob.fetchAll {
		...
		var writeLen, err = clob.connection.Access.dm_build_1478(clob, pos, s, clob.serverEncoding)
		if err != nil {
			return -1, err
		}
...
func (clob *DmClob) Truncate(length int64) error {
	...
	if clob.local || clob.fetchAll {
		...
		clob.length, err = clob.connection.Access.dm_build_1508(&clob.lob, int(length))
		if err != nil {
			return err
		}
		if clob.groupId == -1 {
...
func (clob *DmClob) getSubString(pos int64, len int32) (string, error) {
	...
	if clob.local || clob.inRow || clob.fetchAll {
		...
		return clob.connection.Access.dm_build_1467(clob, int32(pos), len)
	}
}
